...
### 支援実績セクションのブログ記事リンク追加

#### 実施内容
トップページの支援実績（Works）セクションの4つのケーススタディカードから、対応するブログ記事詳細ページへのリンクを追加。

#### 対応するブログ記事（MicroCMS）
| ケーススタディ | ブログID | リンク先 |
|---------------|----------|----------|
| 製造業A社：生成AIによる議事録作成の自動化 | `4287zarb6d` | `/blog/4287zarb6d` |
| 卸売業B社：受発注業務の自動化 | `e5h18mnuqqae` | `/blog/e5h18mnuqqae` |
| 小売業C社：IT導入補助金を活用したPOSシステム刷新 | `zv_2c_zlkd` | `/blog/zv_2c_zlkd` |
| サービス業D社：ペーパーレス化とクラウド移行 | `7cekoid3fd` | `/blog/7cekoid3fd` |

#### 更新ファイル
- `src/components/Works.tsx`
  - `next/link`の`Link`コンポーネントを使用
  - 支援実績データを配列化してmap処理に変更
  - 各カードをブログ記事へのリンクでラップ
  - 「View All Works」ボタンを`/blog`へリンク

- `src/app/globals.css`
  - `.work-card-link`クラスを追加（リンクスタイリング）
  - ホバー時のトランスフォーム効果（`translateY(-4px)`）を追加
  - 既存の画像ズームホバーセレクタをリンク対応に更新

#### コミット
- `fc6e820` - feat: 支援実績カードからブログ記事へのリンクを追加
